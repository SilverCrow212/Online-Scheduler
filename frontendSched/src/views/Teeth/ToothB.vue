...
const handleClickTopBox = (value) => {
  if (selectedChoiceTopBox.value === value) {
    selectedChoiceTopBox.value = null; // Deselect if clicked again
  }
};
const selectedChoiceTopBox = ref(null);
const saveSelectionTopBox = () => {
  props.tooth.topBox = selectedChoiceTopBox.value;  // Save the value to tooth.topBox
  treatmentDialog.value = false;  // Close the dialog
};

const handleClickBotBox = (value) => {
  if (selectedChoiceBotBox.value === value) {
    selectedChoiceBotBox.value = null; // Deselect if clicked again
  }
};
const selectedChoiceBotBox = ref(null);
const saveSelectionBotBox = () => {
  props.tooth.botBox = selectedChoiceBotBox.value;  // Save the value to tooth.topBox
  conditionDialog.value = false;  // Close the dialog
};
</script>
  ...
  <div>
    ...
    <div class="px-1 py-1" color="indigo lighten-4">
      <div class="circle" role="button">
        <div :class="['double_arc', 'arc_top', arcColors.top]" @click="user_details.user_type !== 'patient' ? toggleArcColor('top') : null"></div>
        <div :class="['double_arc', 'arc_right', arcColors.right]" @click="user_details.user_type !== 'patient' ? toggleArcColor('right') : null"></div>
        <div :class="['double_arc', 'arc_bot', arcColors.bottom]" @click="user_details.user_type !== 'patient' ? toggleArcColor('bottom') : null"></div>
        <div :class="['double_arc', 'arc_left', arcColors.left]" @click="user_details.user_type !== 'patient' ? toggleArcColor('left') : null"></div>
        <div :class="['double_arc', 'center', centerColor]"  @click="user_details.user_type !== 'patient' ? toggleCenterColor() : null"></div>
      </div>
    </div>
    <div class="input-container">
      <Button @click="treatmentDialog=true" severity="secondary" :label="treatmentChoices.find(choice => choice.id === tooth.topBox)?.key" class="small-input" :disabled="user_details.user_type === 'patient'"/>
      <Button @click="conditionDialog=true" severity="secondary" :label="conditionChoices.find(choice => choice.id === tooth.botBox)?.key" class="small-input" :disabled="user_details.user_type === 'patient'"/>
    ...
    <Dialog v-model:visible="treatmentDialog" modal  :style="{ width: '35rem' }" :dismissableMask="true">  
        <div class="flex flex-column gap-3">
          <div v-for="choice in treatmentChoices" :key="choice.id" class="flex align-items-center">
            <RadioButton
              v-model="selectedChoiceTopBox"
              :inputId="choice.id"
              name="dynamic"
              :value="choice.id" 
               @click="handleClickTopBox(choice.id)" 
            />
      ...
      <div class="flex justify-content-end gap-2">
        ...
        <Button type="button" label="Select" @click="saveSelectionTopBox"></Button>
      </div>
    </Dialog>
    ...
    <Dialog v-model:visible="conditionDialog" modal  :style="{ width: '35rem' }" :dismissableMask="true"> 
      ...
      <div class="flex flex-column gap-3">
        <div v-for="choice in conditionChoices" :key="choice.id" class="flex align-items-center">
          <RadioButton
            v-model="selectedChoiceBotBox"
            :inputId="choice.id"
            name="dynamic"
            :value="choice.id" 
            @click="handleClickBotBox(choice.id)" 
          />
          <label :for="choice.id" class="ml-2">{{choice.key}} | {{ choice.name }}</label>
        </div>
      </div>
    ...
    <div class="flex justify-content-end gap-2">
      ...
      <Button type="button" label="Select" @click="saveSelectionBotBox"></Button>
    </div>
  </Dialog>
</template>
